...
Automatic cookie import is limited to `huggingface.co` and uses CodexBar's shared cached-cookie/browser-import path.
Ordinary refresh does not open a browser or prompt for Keychain access. Use **Open Hugging Face Billing** from Settings
when a fresh authenticated session is needed.
...
- **Auto** keeps bearer-token spend authoritative and adds the wallet only when the optional billing-page request
  succeeds.
...
- **Web** requests the billing page and can display a balance-only snapshot when API spend is unavailable; when API
  spend is available, only the structured `providerCost.balance` field is merged into it.
...
server-rendered payload omits the wallet and leaves valid API spend unchanged.
